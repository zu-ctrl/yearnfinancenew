...
function CloseIcon(props) {
  ...
  return (
    <SvgIcon className={classes.closeIconStyle}>
      <path
        fill={color}
  ...
  render() {
    ...
    let actions = [
      <IconButton key="close" aria-label="Close" onClick={this.handleClose}>
        <CloseIcon classes={classes} color={theme.snackbar.closeIcon} />
      </IconButton>,
    ]

    switch (type) {
      case 'Error':
        icon = <ErrorIcon color={theme.snackbar.errorIcon} classes={classes} />
        color = theme.snackbar.title
        messageType = t('Snackbar.Error')
        break
      case 'Success':
        icon = <SuccessIcon color={theme.snackbar.successIcon} classes={classes} />
        color = theme.snackbar.title
        messageType = t('Snackbar.Success')
        break
      case 'Warning':
        icon = <WarningIcon color={theme.snackbar.warnIcon} classes={classes} />
        color = theme.snackbar.title
        messageType = t('Snackbar.Warning')
        break
      case 'Info':
        icon = <InfoIcon color={theme.snackbar.infoIcon} classes={classes} />
        color = theme.snackbar.title
        messageType = t('Snackbar.Info')
        break
      case 'Hash':
        icon = <SuccessIcon color={theme.snackbar.hashIcon} classes={classes} />
        color = theme.snackbar.title
        ...
        break
      default:
        icon = <SuccessIcon color={theme.snackbar.successIcon} />
        color = theme.snackbar.title
        messageType = t('Snackbar.Success')
        break
    }
        ...
        message={
          <div style={{ padding: '12px', borderLeft: '5px solid ' + color, borderRadius: '4px' }}>
            ...
            <div style={{ display: 'inline-block', verticalAlign: 'middle', maxWidth: '400px' }}>
              ...
              <Typography variant="body1" style={{ fontSize: '10px', color: theme.snackbar.subtitle }}>
                {message}
              </Typography>
            </div>
          </div>
...
export default withNamespaces()(withStyles(styles, { withTheme: true })(MySnackbar))
