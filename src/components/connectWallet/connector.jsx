...
const styles = (theme) => {
  ...
  return {
    ...
    button: {
      background: colors.wallet.button.bg,
      boxShadow: colors.wallet.button.shadow,
      display: 'flex',
      justifyContent: 'space-between',
      minWidth: '218px',
      padding: '6px 8px',
      ...
      minHeight: '42px',
      '&:hover': {
        background: colors.wallet.button.hover,
      },
    },
    textWallet: {
      color: colors.text,
    ...
    >
      {Object.keys(connectorsByName).map((name) => {
        ...
        if (name === 'MetaMask') {
          ...
          url = require('../../assets/trezor.svg')
        } else if (name === 'Torus') {
          url = require('../../assets/torus.svg')
        ...
        return (
          <div key={name}>
            <Button
              ...
              disabled={disabled}
            >
              <Typography className={classes.textWallet} variant={'h3'}>
                {display}
              </Typography>
...
export default withNamespaces()(withRouter(withStyles(styles)(Connector)))
