  ...
  _calculateDashboard = () => {
    ...
    if (vaults && vaults.length > 0 && assets && assets.length > 0 && prices !== null) {
      ...
      const vaultsInUse = vaults
        ...
        .map((vault) => {
          ...
          vault.vaultGrowth_daily_usd =
            vault.vaultBalance * vault.pricePerFullShare * (vault.apy / 36500) * vault.prices.usd
          vault.vaultGrowth_weekly_usd =
            vault.vaultBalance * vault.pricePerFullShare * (vault.apy / 5200) * vault.prices.usd
          vault.vaultGrowth_yearly_usd =
            ((vault.vaultBalance * vault.pricePerFullShare * vault.apy) / 100) * vault.prices.usd

          vault.ethBalance = vault.vaultBalance * vault.pricePerFullShare * vault.prices.eth
          vault.vaultGrowth_daily_eth =
            vault.vaultBalance * vault.pricePerFullShare * (vault.apy / 36500) * vault.prices.eth
          vault.vaultGrowth_weekly_eth =
            vault.vaultBalance * vault.pricePerFullShare * (vault.apy / 5200) * vault.prices.eth
          vault.vaultGrowth_yearly_eth =
            ((vault.vaultBalance * vault.pricePerFullShare * vault.apy) / 100) * vault.prices.eth
      ...
      const portfolioGrowthDailyPerc_usd =
        ((vaultBalances.vaultGrowth_daily_usd + earnBalances.earnGrowth_daily_usd) * 100) /
        (vaultBalances.vaultBalance_usd + earnBalances.earnBalance_usd)
      const portfolioGrowthWeeklyPerc_usd =
        ((vaultBalances.vaultGrowth_weekly_usd + earnBalances.earnGrowth_weekly_usd) * 100) /
        (vaultBalances.vaultBalance_usd + earnBalances.earnBalance_usd)
      const portfolioGrowthYearlyPerc_usd =
        ((vaultBalances.vaultGrowth_yearly_usd + earnBalances.earnGrowth_yearly_usd) * 100) /
        (vaultBalances.vaultBalance_usd + earnBalances.earnBalance_usd)
      ...
      const portfolioGrowthDailyPerc_eth =
        ((vaultBalances.vaultGrowth_daily_eth + earnBalances.earnGrowth_daily_eth) * 100) /
        (vaultBalances.vaultBalance_eth + earnBalances.earnBalance_eth)
      const portfolioGrowthWeeklyPerc_eth =
        ((vaultBalances.vaultGrowth_weekly_eth + earnBalances.earnGrowth_weekly_eth) * 100) /
        (vaultBalances.vaultBalance_eth + earnBalances.earnBalance_eth)
      const portfolioGrowthYearlyPerc_eth =
        ((vaultBalances.vaultGrowth_yearly_eth + earnBalances.earnGrowth_yearly_eth) * 100) /
        (vaultBalances.vaultBalance_eth + earnBalances.earnBalance_eth)
